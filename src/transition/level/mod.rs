...
impl LevelData {
    ...
    pub fn act(
        ...
        actor_queue: &mut actor::ActorQueue,
        actor_message_queue: &mut ActorMessageQueue,
    ) {
        let animated_frames =
...
pub mod ffi {
    ...
    #[no_mangle]
    pub extern "C" fn fn_level_data_load(
        file: *mut FnFile,
        hero: *mut FnHeroData,
        ...
        assert!(!file.is_null());
        let file: &mut FnFile = unsafe { &mut (*file) };

        assert!(!hero.is_null());
        let hero: &mut FnHeroData = unsafe { &mut (*hero) };

        assert!(!tilecache.is_null());
        let tilecache: &FnTileCache = unsafe { &(*tilecache) };

        let mut raw: Option<&mut FnLevelRaw> = if !raw.is_null() {
            Some(unsafe { &mut (*raw) })
        } else {
            None
        };

        match FnLevelData::load(
            &mut file.0,
            hero,
            tilecache,
            ...
            Ok(level) => Box::into_raw(Box::new(level)),
            Err(e) => {
                warn!("{}", e);
                std::ptr::null_mut()
            }
    ...
    pub extern "C" fn fn_level_data_act(
        ...
        d.act(hero, actor_queue, actor_message_queue);
    }
    ...
    pub extern "C" fn fn_level_data_blit(
        ...
        hero: *mut FnHeroData,
        draw_collision_bounds: bool,
        targetrect: FnGeometry,
        sourcerect: FnGeometry,
        backdrop1: *const FnTexture,
        backdrop2: *const FnTexture,
    ) {
        assert!(!ptr.is_null());
        let d: &mut FnLevelData = unsafe { &mut (*ptr) };
        ...
        let backdrop1 = if !backdrop1.is_null() {
            Some(unsafe { &(*backdrop1) })
        } else {
            None
        };

        let backdrop2 = if !backdrop2.is_null() {
            Some(unsafe { &(*backdrop2) })
        } else {
            None
        };
