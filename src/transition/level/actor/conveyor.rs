...
fn create(
    ...
    let direction = match general.actor_type {
        ActorType::ConveyorLeftMovingRightEnd => HorizontalDirection::Left,
...
fn act(
    ...
    if hero_geometry.x + hero_geometry.w as i16 > general.position.x
        && hero_geometry.x < general.position.x + general.position.w as i16
...
fn blit(
    ...
    for i in 0..num_elements {
        if i == num_elements - 1 {
            ...
            tile = tilecache
                .get_tile(
                    SOLID_CONVEYORBELT_RIGHTEND + specific.current_frame,
                )
                .unwrap();
        } else if i == 1 {
            ...
            tile = tilecache
                .get_tile(
                    SOLID_CONVEYORBELT_CENTER + specific.current_frame % 2,
                )
                .unwrap();
        }
