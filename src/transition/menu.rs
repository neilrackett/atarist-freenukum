...
pub mod ffi {
    ...
    pub type FnMenu = super::Menu;
    pub type FnMenuEntry = super::MenuEntry;

    #[no_mangle]
    pub extern "C" fn fn_menu_create(
        header: *const c_char,
    ) -> *mut FnMenu {
        assert!(!header.is_null());
        let header = {
            match unsafe { CStr::from_ptr(header) }.to_str() {
                Ok(header) => header.to_string(),
                Err(e) => {
                    eprintln!("Couldn't read header: {:?}.", e);
                    return std::ptr::null_mut();
                }
            }
        };

        Box::into_raw(Box::new(FnMenu::new(header)))
    }

    #[no_mangle]
    pub extern "C" fn fn_menu_append_entry(
        ptr: *mut FnMenu,
        shortcut: c_char,
        name: *mut c_char,
    ) {
        ...
        let shortcut = shortcut as u8 as char;

        menu.append(FnMenuEntry { shortcut, name });
    }

    #[no_mangle]
    pub extern "C" fn fn_menu_get_choice(
        ptr: *mut FnMenu,
        ...
        menu.get_choice(&mut screen, tilecache, texture_creation_paramns)
            as i8
    }

    #[no_mangle]
    pub extern "C" fn fn_menu_free(ptr: *mut FnMenu) {
        if !ptr.is_null() {
            unsafe {
                Box::from_raw(ptr);
            }
